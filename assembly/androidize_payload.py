...
def discover_external_plugins():
    """Find user plugin packages outside the upstream repo.

    Scans the configured roots (default E:\\code top level) for directories
    whose package.json declares a name under the @dsh-external scope — the
    user's out-of-tree cordis plugin convention. Returns [(dir, fullName)].
    """
    import json as _json
    import glob as _glob
    roots = os.environ.get("DSH_ANDROID_PLUGIN_ROOTS", r"E:\code")
    found = []
    seen = set()
    for root in roots.split(os.pathsep):
        if not os.path.isdir(root):
            continue
        for pkg_dir in _glob.glob(os.path.join(root, "*",)):
            ...
            try:
                with open(manifest_path, encoding="utf-8") as handle:
                    name = _json.load(handle).get("name")
            except (OSError, ValueError):
                continue
            if isinstance(name, str) and name.startswith("@dsh-external/"):
                found.append((pkg_dir, name))
                seen.add(pkg_dir)
    return sorted(found, key=lambda item: item[1])


def external_plugin_patch_fragment(plugins):
    """Emit the loader insert rows for discovered @dsh-external plugins.

    Written next to this script as generated/user-plugins.patch.yml;
    make_runtime_zip.py merges it into the android.patch.yml zip entry so the
    repo-tracked patch file never carries machine-generated rows.
    prompt-loader's state file is pinned into $DSH_HOME so APK updates
    (which re-extract runtime.zip over the payload tree) cannot wipe it.
    """
    rows = []
    for _dir, name in plugins:
        row = {"id": name.split("/")[-1], "name": name}
        if name == "@dsh-external/dsh-prompt-loader":
            row["config"] = {
                "stateFile": "/data/data/dev.dsh.spike/files/.dsh/prompt-loader-state.json",
            }
        rows.append(row)
    lines = ["# GENERATED by androidize_payload.install_user_plugins — do not edit;",
             "# source of truth: dirs under DSH_ANDROID_PLUGIN_ROOTS with @dsh-external/* names.",
             "- insert:"]
    for row in rows:
    ...
    out_path = os.path.join(out_dir, "user-plugins.patch.yml")
    with open(out_path, "w", encoding="utf-8", newline="\n") as handle:
        handle.write("\n".join(lines) + "\n")
    print(f"wrote {out_path} ({len(rows)} insert rows)")
    return out_path
    ...
    if os.path.isdir(src_root):
        for name in os.listdir(src_root):
            ...
            if os.path.isfile(pkg_json):
                with open(pkg_json, encoding="utf-8") as handle:
                    pkg_name = json.load(handle).get("name", name)
                sources.append((os.path.join(src_root, name), name, pkg_name))
    else:
        print(f"WARN: user plugin source not found: {src_root}")
    for pkg_dir, full_name in discover_external_plugins():
        sources.append((pkg_dir, full_name.split("/")[-1], full_name))
    if not sources:
        return
    nm = os.path.join(payload, "node_modules")
    import shutil as _shutil
    copied = []
    for src_dir, _short, pkg_name in sources:
        dst = os.path.join(nm, *pkg_name.split("/"))
        ...
        vendor_missing_runtime_deps(src_dir, nm, pkg_name)
    external = [(src_dir, pkg_name) for src_dir, _short, pkg_name in sources if pkg_name.startswith("@dsh-external/")]
    external_plugin_patch_fragment(external)
    if not copied:
        return
...
def vendor_missing_runtime_deps(src_dir, payload_nm, pkg_name):
    """Copy plugin-local node_modules entries the payload root lacks.

    Out-of-tree plugins resolve their own deps from their dev node_modules on
    PC (e.g. unscoped `schemastery`, while the payload only ships the vendored
    `@deepseek-ai/schemastery`). Names ALREADY present in the payload root are
    skipped so newer payload versions never get shadowed by stale plugin pins;
    pnpm symlinked entries are materialized by copytree's default behaviour.
    """
    import shutil as _shutil
    local_nm = os.path.join(src_dir, "node_modules")
    if not os.path.isdir(local_nm):
        return
    vendored = []
    for entry in os.listdir(local_nm):
        if entry in (".bin", "@types") or entry.startswith("."):
            continue
        src = os.path.join(local_nm, entry)
        dst = os.path.join(payload_nm, entry)
        if os.path.lexists(dst):
            continue
        if os.path.isdir(src):
            _shutil.rmtree(dst, ignore_errors=True)
            try:
                # Drop the entry's own nested node_modules: those hold pnpm
                # junctions Windows cannot materialize, and every real runtime
                ...
                _shutil.rmtree(dst, ignore_errors=True)
                print(f"skip vendoring {entry} for {pkg_name}: {str(error).splitlines()[0][:120]}")
                continue
        else:
            _shutil.copy2(src, dst)
        vendored.append(entry)
    if vendored:
        print(f"vendored runtime deps for {pkg_name}: {', '.join(sorted(vendored))}")


def install_user_agent_presets(payload):
    """Stage the user's agent presets (router-standard, router-spec, …) from
    the PC harness home into payload/config/user-agent-presets/.

    DshService mirrors this directory to $DSH_HOME/.agent-presets after each
    runtime extraction; DSH discovers presets as first-level directories.
    """
    import shutil as _shutil
    src_root = os.path.join(os.path.expanduser("~"), ".dsh", ".agent-presets")
    if not os.path.isdir(src_root):
        print(f"WARN: agent preset source not found: {src_root}")
        return
    dst_root = os.path.join(payload, "config", "user-agent-presets")
    ...
    for name in os.listdir(src_root):
        ...
        count += 1
        print(f"staged user agent preset {name}")
    if count:
        print(f"staged {count} user agent presets -> config/user-agent-presets")
...
def main():
    ...
    install_user_agent_presets(payload)
